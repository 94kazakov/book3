...
function strength(batch){
	if (batch.length < 5){
		return
	}
  change = batch[0].price - batch[4].price

  if (batch[4].tweet.match(/fun/i)) {
    words['fun'][1] += 1
    words['fun'][0] += change/words['fun'][1]
  }
  else if (batch[4].tweet.match(/apple/i)) {
    words['apple'][1] += 1
    words['apple'][0] += change/words['apple'][1]
  }
  else if (batch[4].tweet.match(/google/i)) {
    words['google'][1] += 1
    words['google'][0] += change/words['google'][1]
  }
  else if (batch[4].tweet.match(/game/i)) {
    words['game'][1] += 1
    words['game'][0] += change/words['game'][1]
  }
  else if (batch[4].tweet.match(/news/i)) {
    words['news'][1] += 1
    words['news'][0] += change/words['news'][1]
  }
  else if (batch[4].tweet.match(/money/i)) {
    words['money'][1] += 1
    words['money'][0] += change/words['money'][1]
  }
  else if (batch[4].tweet.match(/good/i)) {
    words['good'][1] += 1
    words['good'][0] += change/words['good'][1]
  }
  else if (batch[4].tweet.match(/actor/i)) {
    words['actor'][1] += 1
    words['actor'][0] += change/words['actor'][1]
  }
  else if (batch[4].tweet.match(/movies/i)) {
    words['movies'][1] += 1
    words['movies'][0] += change/words['movies'][1]
  }
  else if (batch[4].tweet.match(/tech/i)) {
    words['tech'][1] += 1
    words['tech'][0] += change/words['tech'][1]
  }
  else if (batch[4].tweet.match(/music/i)) {
    words['music'][1] += 1
    words['music'][0] += change/words['music'][1]
  }
  else if (batch[4].tweet.match(/people/i)) {
    words['people'][1] += 1
